...
class Attention(Module):
    ...
    def __init__(self, dim, num_heads=8, attention_dropout=0.1, projection_dropout=0.1):
        ...
        # LSA: learnable temperature parameter as per Locality Self-Attention (LSA) paper
        self.temperature = torch.nn.Parameter(torch.log(torch.tensor(head_dim ** -0.5)))
    ...
    def forward(self, x):
        ...
        # LSA: scale dot-product attention scores by learnable temperature parameter
        attn = (q @ k.transpose(-2, -1)) * self.temperature.exp()
        # LSA: mask out diagonal elements to prevent self-attention to the same token (locality bias)
...
class MaskedAttention(Module):
    ...
    def forward(self, x, mask=None):
        ...
        # LSA: mask out diagonal elements to prevent self-attention to the same token (locality bias)
        mask_eye = torch.eye(N, device=x.device, dtype=torch.bool)
        attn = attn.masked_fill(mask_eye[None, None, :, :], float('-inf'))
